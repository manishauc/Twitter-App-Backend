...
public class AuthServiceImpl implements AuthService {
    ...
    public RegisterDto register(RegisterDto registerDto) {
    	...
    	//Name validation 
    	 if((registerDto.getName().length() <1) || (registerDto.getEmail().length()<1) || (registerDto.getPassword().length()<1) ){
    		 throw new BlogAPIException(HttpStatus.BAD_REQUEST, "All fields are mandetory.");
    	 }
    	//Email validator
    	    String regex = "^(.+)@(.+)$";
